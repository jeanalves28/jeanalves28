...
export function bgColor(theme: ToggleTheme, path?: string): string {
	...
	if (/^\/(contact|projects\/\w+)/.test(path ?? '')) {
		classes = [
			theme === 'system'
				? 'bg-image-gradient-system'
				: theme === 'dark'
					? 'bg-image-gradient-dark'
					: 'bg-image-gradient-light',
		];
	}

	if (path?.startsWith('/contact')) {
		classes = [...classes, 'bg-radial-[circle]'];

		return classes.join(' ');
	}

	if (path?.startsWith('/projects/')) {
		classes = [...classes, 'bg-radial-[circle_at_50%_40%]'];
