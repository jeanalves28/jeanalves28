...
import { contextNavMenuDropBox, contextTheme, HeaderScrolling } from '@/custom';
...
export default function Header() {
	...
	return (
		<header
			id="nav-header-fixed"
			ref={headerRef}
			className="fixed duration-500 transition-translate right-0 left-0 top-0 z-2 flex items-center pc:justify-center justify-between gap-x-10 py-5 backdrop-blur-sm"
		>
			{headerContent(path)
				.map(({ title, path }, i) => (
					<Link
