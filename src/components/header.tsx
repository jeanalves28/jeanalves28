...
export default function Header () {
	...
	useEffect(() => {
		const element = document.querySelector<HTMLElement>('.bg-wallpaper-dev');

		if (element){
			widthScreen = element.offsetWidth;
		}
	}, []);

	return (
		<header className="fixed left-0 right-0 top-0 z-2 flex items-center justify-center gap-x-10 py-5 backdrop-blur-sm">
			{ widthScreen >= 640
				? <Button />
					...
					.map(({ title, path }, i) => (
						<Link
							key={`nav-${i + 1}-${title.toLowerCase()
								.replace(/\s/g, '-')}`}
							href={path}
							className={`title-header smartphone:hidden sm:block ${titleTheme}`}
							scroll={false}
